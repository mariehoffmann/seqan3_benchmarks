 ...
 */
template<long long int N, short unsigned int POW1 = 5, short unsigned int POW2 = 8, bool GAP_FLAG=0>
void benchmark2()
{
    ...
    {
        if (LOG_LEVEL_<[benchmark]>) std::cout << "seq_len = " << seq_len << std::endl;
        // reset durations
        ...
        for (long unsigned int round = 0; round < N; ++round)
        {
            ...
            //fill vector with A
            if (LOG_LEVEL_<[benchmark]>) std::cout << "start filling sequences ...\n";
            ...
            // copy for others
            if (LOG_LEVEL_<[benchmark]>) std::cout << "start copying sequences ...\n";
            s_al = s_gv;
            s_as = s_gv;
            if (LOG_LEVEL_<[benchmark]>) std::cout << "done.\n";
            
            if (LOG_LEVEL_<[benchmark]>) std::cout << "init seq length is : " << s_gv.size() << std::endl;
            // sample gap lengths
            gaps.resize(seq_len);
            ...
            // generic method
            <[structure]><inner_type> gap_dec(&seq);
            ...
            if (GAP_FLAG)
            {
                ...
                for (size_type i = gaps.size()-1; i != 0; --i)
                {
                    ...
                    if (gaps[i] > 0)
                    {
                        if (LOG_LEVEL_<[benchmark]>) std::cout << "insert gap (" << i << ", " << gaps[i] << ") into AS ...\n";
                        as.insert_gap(i, gaps[i]);
                        if (LOG_LEVEL_<[benchmark]>) print_sequence<anchor_set<std::vector<dna15>>>(as);
                        
                        if (LOG_LEVEL_<[benchmark]>) std::cout << "and into AL ...\n";
                        al.insert_gap(i, gaps[i]);
                        if (LOG_LEVEL_<[benchmark]>) print_sequence<anchor_list<std::vector<dna15>>>(al);
                        
                        if (LOG_LEVEL_<[benchmark]>) std::cout << "and into GV ...\n";
                        gv.insert_gap(i, gaps[i]);
                        if (LOG_LEVEL_<[benchmark]>) print_sequence<gap_vector>(gv);
                        
                        if (LOG_LEVEL_<[benchmark]>) std::cout << "and into GS ...\n";
                ...
                al.resize(seq_len);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "... done for AL\n";
                as.resize(seq_len);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "... done for AS\n";
                gv.resize(seq_len);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "... done for GV\n";
                gs.resize(seq_len);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "... done for GS\n";
                if (LOG_LEVEL_<[benchmark]>) std::cout << "aligned size: " << as.size() << std::endl;
            }
            
            if (LOG_LEVEL_<[benchmark]>) print_sequence<anchor_set<inner_type>>(as);
            if (LOG_LEVEL_<[benchmark]>) print_sequence<anchor_list<inner_type>>(al);
            if (LOG_LEVEL_<[benchmark]>) print_sequence<gap_vector>(gv);
            ...
            for (size_type j = 0; j < R; ++j)
            {
                ...
                if (LOG_LEVEL_<[benchmark]>) std::cout << "seq_len = " << seq_len << ", insert gap at pos " << pos << std::endl;
                // ins/del on anchor list (AS)
                t1 = std::chrono::high_resolution_clock::now();
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tas.insert...\n";
                as.insert_gap(pos, 1);
                as.insert_gap(pos, 2);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tas.erase...\n";
                ...
                t1 = std::chrono::high_resolution_clock::now();
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tal.insert...\n";
                al.insert_gap(pos, 1);
                al.insert_gap(pos, 2);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tal.erase...\n";
                ...
                t1 = std::chrono::high_resolution_clock::now();
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tgv.insert...\n";
                gv.insert_gap(pos, 1);
                gv.insert_gap(pos, 2);
                if (LOG_LEVEL_<[benchmark]>) std::cout << "\tgv.erase...\n";
        ...
        for (auto i = 0; i < I; ++i)
        {
            if (LOG_LEVEL_<[benchmark]>) std::cout << "compute stats for method " << i << std::endl;
            avg_duration = avg<R*N>(&durations[i], quantile);
            stddev_duration = stddev<R*N>(&durations[i], quantile);
            if (LOG_LEVEL_<[benchmark]>) std::cout << "avg duration time of approach " << i << ": " << avg_duration << "\u00B1" << stddev_duration << " ns\n";
