...
namespace seqan3 {
    ...
    template <typename inner_type, unsigned short block_size=256>
    ...
    struct anchor_blocks_256
    {

    private:
        using gap_decorator_t   = anchor_blocks_256;
    ...
    public:
        ...
        constexpr anchor_blocks_256()
        {
            data = std::shared_ptr<data_t>(new data_t{});
        };

        constexpr anchor_blocks_256(anchor_blocks_256 const &) = default;

        constexpr anchor_blocks_256 & operator=(anchor_blocks_256 const &) = default;

        constexpr anchor_blocks_256 (anchor_blocks_256 && rhs) = default;

        constexpr anchor_blocks_256 & operator=(anchor_blocks_256 && rhs) = default;

        ~anchor_blocks_256() {data->gap_block_list.clear(); data->gap_sums.clear();};

        constexpr anchor_blocks_256(inner_type * sequence): data{new data_t{sequence}}
        {
